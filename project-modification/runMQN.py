...
from stable_baselines3 import MQN
...
model = MQN("MlpPolicy", env, verbose=1, device='cuda', seed=666)
callback = SaveOnBestTrainingRewardCallback(check_freq=10, log_dir=log_dir)
model.learn(total_timesteps=timesteps, callback=callback) #25000)
model.save("mqn_lunarlander-v2") #cartpole")
plot_results([log_dir], timesteps, results_plotter.X_TIMESTEPS, "MQN LunarLander")
#plt.show()
plt.savefig('mqn_curve.png')
mean_reward, std_reward = evaluate_policy(model, model.get_env(), n_eval_episodes=10)
print('mean reward = {}, std_reward = {}'.format(mean_reward, std_reward))
del model # remove to demonstrate saving and loading

model = PPO.load("mqn_lunarlander-v2")
...
writer = imageio.get_writer(log_dir + 'mqn_lunarlander-v2.mp4', fps=20)
